...
export function StatePanel({ actionLabel, description, kind, title }: StatePanelProps) {
  const Icon = kind === "empty" ? CompassIcon : AlertIcon;

  return (
    <section className={cn("state-panel", `state-panel--${kind}`)} role={kind === "error" ? "alert" : "status"}>
      ...
      {actionLabel ? (
        <Button aria-disabled="true" className="state-panel__action" disabled variant="secondary">
...
export function LoadingState({ label = "Les vibes arrivent…" }: { label?: string }) {
  return (
...
export function Skeleton({ className, children }: { children?: ReactNode; className?: string }) {
  return (
